	...
	<div class="card">
		...
		<div class="card-body">
			...
			<div class="table-responsive">
				<form class="form-horizontal" action="{{route('client.products.update',['id'=>$dataInfo->id])}}" method="post" enctype= "multipart/form-data">
					{!! csrf_field() !!}
					<input name="_method" type="hidden" value="PUT">

					<?php
					$parentCategory = old('parent_category',$dataInfo->getCategory->id);
					$category_id =  old('category_id',$dataInfo->getCategory->id);
					//echo $parentCategory."===".$category_id;
					?>
					<table class="table table-bordered" style="width: 100%;">
						<tr>
							...
							<td>
								<select class="form-control" id="parent_category" name="parent_category" placeholder="">
									 <option value="{{$pat->id}}">{{$pat->name}}</option>
									@foreach ($categories as $data)
											
										<option value="{{$data->id}}" {{ (old('client_id') == $data->id)?" SELECTED":"" }}>{{$data->name}}</option>
									}
									}
									@endforeach
								</select>
...
jQuery(document).ready(function() {
	jQuery(document).on("change","#parent_category",function(event){
		...
		if(categoryId && categoryId!=""){
			Helpers.callAjax(APP_URL+'/client/category/json/'+categoryId,'GET',{},'json',function(responseType,response){
				switch(responseType){
					case "success":
						if(response.success){
