...
@include ('client.partials.form-error-messages')
	<div class="btn-group pull-right">
		<a href="{{route('client.products.create')}}" class="btn btn-primary"><i class="fa fa-plus-circle" aria-hidden="true"></i>  New Product</a>
	</div>
					  ...
					  <tbody>
					  	@foreach ($dataList as $row)
							...
							<tr>
							  ...
							  <td width="100px">
									<a href="{{route('client.products.show',['id'=>$row->id])}}"><i class="fa fa-info-circle" title="View"></i></a>
									<a href="{{route('client.products.edit',['id'=>$row->id])}}"><i class="fa fa-edit" title="Edit"></i></a>

									<i class="fa fa-trash-o btn-delete" title="Remove" data-id="{{$row->id}}"></i>

									<form id="delete-form-{{$row->id}}" action="{{ route('client.products.delete' , ['id'=>$row->id] ) }}" method="POST" style="display: none;">
										{{ csrf_field() }}
										{{ method_field('DELETE') }}
										{{ csrf_field() }}
									</form>
...
<script type="text/javascript">
	jQuery(document).ready(function(){
		var a=jQuery('#table-products').DataTable();	
		alert(a);
