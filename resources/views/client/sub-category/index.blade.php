...
@include ('client.partials.form-error-messages')
	<div class="btn-group pull-right">
		<a href="{{route('client.sub-categories.create')}}" class="btn btn-primary"><i class="fa fa-plus-circle" aria-hidden="true"></i>  New Sub Category</a>
	</div>
					  ...
					  <tbody>
						@foreach ($categories as $row)
							<tr>
							  ...
							  <td>{{$row->getParentCategory->name}} <i class="fa fa-arrow-right"></i> {{$row->name}}</td>
							
							  <td>{{$row->getStatus->name}}</td>
							  <td>{{$row->created_at}}</td>
							  <td width="100px">
									<a href="{{route('client.sub-categories.show',['id'=>$row->id])}}"><i class="fa fa-info-circle" title="View"></i></a>
									<a href="{{route('client.sub-categories.edit',['id'=>$row->id])}}"><i class="fa fa-edit" title="Edit"></i></a>

									<i class="fa fa-trash-o btn-delete" title="Remove" data-id="{{$row->id}}"></i>

									<form id="delete-form-{{$row->id}}" action="{{ route('client.sub-categories.delete' , ['id'=>$row->id] ) }}" method="POST" style="display: none;">
										{{ csrf_field() }}
										{{ method_field('DELETE') }}
										{{ csrf_field() }}
									</form>
