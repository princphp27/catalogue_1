...
<div class="col-xl-12">
	...
	<div class="btn-group pull-right">
		<a href="{{route('admin.categories.create')}}" class="btn btn-primary"><i class="fa fa-plus-circle" aria-hidden="true"></i>  New Category</a>
	</div>
						...
						@foreach ($categories as $row)
							<tr>
							  ...
							  <td>{{$row->created_at}}</td>
							   <td width="100px">
									<a href="{{route('admin.categories.show',['id'=>$row->id])}}"><i class="fa fa-info-circle" title="View"></i></a>
									<a href="{{route('admin.categories.edit',['id'=>$row->id])}}"><i class="fa fa-edit" title="Edit"></i></a>
									<i class="fa fa-trash-o btn-delete" title="Remove" data-id="{{$row->id}}"></i>
									<form id="delete-form-{{$row->id}}" action="{{ route('admin.categories.delete' , ['id'=>$row->id] ) }}" method="POST" style="display: none;">
										{{ csrf_field() }}
										{{ method_field('DELETE') }}
										{{ csrf_field() }}
									</form>
