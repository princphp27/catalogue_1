...
@include ('admin.partials.form-error-messages')
	<div class="btn-group pull-right">
		<a href="{{route('admin.sub-categories.create',$id)}}" class="btn btn-primary"><i class="fa fa-plus-circle" aria-hidden="true"></i>  New Sub Category</a>
						...
						@foreach ($categories as $row)
							...
							<tr>
							 ...
							 @if(($row->status)==1)
							  ...
							  <td width="100px">
									<a href="{{route('admin.sub-categories.show',['id'=>$row->id])}}"><i class="fa fa-info-circle" title="View"></i></a>
									<a href="{{route('admin.sub-categories.edit',['id'=>$row->id])}}"><i class="fa fa-edit" title="Edit"></i></a>

									<i class="fa fa-trash-o btn-delete" title="Remove" data-id="{{$row->id}}"></i>

									<form id="delete-form-{{$row->id}}" action="{{ route('admin.sub-categories.delete' , ['id'=>$row->id] ) }}" method="POST" style="display: none;">
										{{ csrf_field() }}
										{{ method_field('DELETE') }}
										{{ csrf_field() }}
									</form>
